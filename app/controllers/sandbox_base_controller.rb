class SandboxBaseController < ApplicationController
  ...
  def authenticate_with_cwds
    ...
    token = Cwds::Authentication.token_generation(params[:accessCode], ENV.fetch('PERRY_BASE_URL'))
    ...
    if Cwds::Authentication.token_validation(new_token, ENV.fetch('PERRY_BASE_URL'))
      puts "token validated success #{new_token}"
      session[:token] = new_token

      profile = Cwds::Authentication.store_user_details_from_token(new_token, ENV.fetch('PERRY_BASE_URL'))
      ...
      redirect_to Cwds::Authentication.authentication_url(ENV.fetch('LOGIN_BASE_URL'), request.original_url)
    end
  end
